...
import { Mail, Trash2, ArrowLeft, X, Send } from "lucide-react";
import { useRouter } from "next/navigation";

const cardClass =
  "bg-card rounded-[5px] border border-border overflow-hidden shadow-sm dark:shadow-none";
          ...
          <div className="p-6 border-b border-border flex items-center justify-between bg-muted/10">
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                ...
                >
                  <span className="absolute left-1.5 top-1 font-bold text-[8px] text-white select-none">
                    {isRead ? "ON" : ""}
                  </span>
                  <span className="absolute right-1.5 top-1 font-bold text-[8px] text-muted-foreground select-none">
                    {!isRead ? "OFF" : ""}
                  </span>
                  <div
                    className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isRead ? "left-7" : "left-1"}`}
                  />
                </button>
              </div>
        ...
        {!isReplying ? (
          <div className={cardClass + " p-6"}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-[13px]">
                    Compose an email response to the user.
                  </p>
                </div>
              </div>
              ...
              >
                <Send size={16} className="-rotate-45 -translate-y-0.5" /> Write
                Reply
              </button>
            </div>
          </div>
            ...
            <div className="p-6 bg-card rounded-[5px] border border-border shadow-md">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="space-y-3 px-2">
                  <div className="flex items-center gap-3">
                    <span className="w-16 text-[13px] text-muted-foreground font-medium">
                      From:
                    </span>
                    <span className="text-[13px] text-foreground font-bold italic">
                      Admin
                    </span>
                  </div>
              ...
              <div className="bg-muted/30 border border-border rounded-t-[5px] p-2 flex items-center gap-1">
                {["B", "I", "U", "H1", "H2", "Link", "Img", "Undo", "Redo"].map(
                  (btn) => (
                    <button
                      key={btn}
                      className="h-8 w-8 flex items-center justify-center rounded-[3px] text-muted-foreground hover:bg-muted hover:text-primary transition-all font-bold text-[13px]"
                    >
                      {btn}
                    </button>
                  ),
