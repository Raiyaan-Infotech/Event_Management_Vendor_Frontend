...
export default function AddTestimonialContent() {
  ...
  const handleCropComplete = (croppedBase64: string) => {
    ...
    // Convert base64 to File for upload on save
    fetch(croppedBase64)
      .then((r) => r.blob())
    ...
    <div className="h-[calc(100vh-86px)] overflow-y-auto px-6 py-8 custom-scrollbar bg-[#F8FAFC] dark:bg-black">
      <div className="max-w-[1700px] mx-auto mb-10 space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-[var(--vendor-radius-panel)] bg-orange-500/10 flex items-center justify-center text-orange-600 shadow-sm border border-orange-500/10">
            <Star size={28} fill="currentColor" />
          </div>
          <div>
            <h1 className="text-3xl font-black text-[var(--vendor-text)] font-poppins uppercase tracking-tighter">Add New Testimonial</h1>
            <p className="text-sm text-[var(--vendor-text-muted)] font-medium tracking-wide">Enter the details of the client feedback to showcase on your website.</p>
          </div>
        </div>
      </div>
                  ...
                  {preview ? (
                    <div className="relative w-full h-full min-h-[180px] flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <Button variant="destructive" size="icon" onClick={(e) => { e.stopPropagation(); setPreview(""); setImageFile(null); }} className="w-12 h-12 rounded-[var(--vendor-radius-panel)]">
                          <X size={20} />
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-5 text-center">
                      ...
                      <div>
                        <p className="text-xs font-black uppercase tracking-wide text-gray-800 dark:text-gray-200">Upload Customer Photo</p>
                        <p className="text-[9px] font-bold text-[var(--vendor-text-muted)] mt-2 uppercase tracking-wide">Recommended: 1:1 Aspect Ratio</p>
                      </div>
                    </div>
                  )}
