  ...
  const advanceSliderColumns: Column<VendorSlider>[] = [
    {
      ...
      sortable: true,
      render: (item) => (
        <span className="text-[12px] font-black text-[var(--vendor-text-muted)] group-hover:text-indigo-500 transition-colors tracking-tighter">
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex flex-col gap-1 text-gray-700 font-bold">
          ...
          <p className="text-[10px] text-[var(--vendor-text-muted)] font-medium line-clamp-1 max-w-[240px] pl-4">{item.description ?? ""}</p>
        </div>
      ),
    },
    {
      ...
      render: (item) => (
        <div className="flex flex-col gap-0.5">
          <span className="text-[13px] font-bold text-gray-600 dark:text-[var(--vendor-text-muted)] leading-none">{item.button_label}</span>
    ...
    },
    {
      ...
      sortable: true,
      render: (item) => (
        <span className="text-[var(--vendor-control-text)] font-semibold text-gray-500 uppercase tracking-tight capitalize">
        ...
        filterContent={
          <div className="space-y-4">
            <div className="space-y-2">
              <p className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide ml-1">Slider Status</p>
              <Select value={filterStatus} onValueChange={setFilterStatus}>
                ...
                <SelectContent className="rounded-[var(--vendor-radius-control)] border-[var(--vendor-border)]">
                  ...
                  <SelectItem value="Published" className="text-[var(--vendor-control-text)] font-semibold">Published</SelectItem>
                  <SelectItem value="Draft" className="text-[var(--vendor-control-text)] font-semibold">Draft</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <p className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide ml-1">Content Alignment</p>
              <Select value={filterAlignment} onValueChange={setFilterAlignment}>
                <SelectTrigger className="h-10 rounded-[var(--vendor-radius-control)] text-[var(--vendor-control-text)] font-semibold border-[var(--vendor-border)] bg-gray-50/50 focus:ring-0">
                  <SelectValue placeholder="All Alignments" />
                </SelectTrigger>
                <SelectContent className="rounded-[var(--vendor-radius-control)] border-[var(--vendor-border)]">
                  <SelectItem value="All" className="text-[var(--vendor-control-text)] font-semibold">All Alignments</SelectItem>
                  <SelectItem value="left" className="text-[var(--vendor-control-text)] font-semibold uppercase">Left</SelectItem>
                  <SelectItem value="center" className="text-[var(--vendor-control-text)] font-semibold uppercase">Center</SelectItem>
                  <SelectItem value="right" className="text-[var(--vendor-control-text)] font-semibold uppercase">Right</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <Button variant="ghost" onClick={() => { setFilterStatus("All"); setFilterAlignment("All"); }} className="w-full h-10 rounded-[var(--vendor-radius-control)] text-[var(--vendor-control-text)] font-semibold uppercase tracking-wide text-rose-500 hover:bg-rose-50 transition-all">
              Reset Filters
            </Button>
          </div>
        }
      ...
      <Dialog open={!!sliderToDelete} onOpenChange={(open) => !open && setSliderToDelete(null)}>
        <DialogContent className="sm:max-w-[420px] rounded-[var(--vendor-radius-panel)] p-0 overflow-hidden border-none shadow-2xl shadow-rose-900/10">
          <div className="bg-gradient-to-b from-rose-50 to-white dark:from-rose-500/10 dark:to-[#111827] p-10 flex flex-col items-center text-center">
            ...
            <DialogTitle className="text-[var(--vendor-title-text)] font-bold text-[var(--vendor-text)] uppercase tracking-tighter">Delete Slider?</DialogTitle>
