...
export default function EmailTemplateManagementContent() {
  ...
  useEffect(() => {
    ...
    if (savedTemplates) {
      ...
      // Default initial templates if nothing in localStorage
      const initialTemplates = [
        {
          id: 1,
          name: "Welcome email",
          details: "Standard welcome onboarding email for new users.",
          category: "Welcome",
        }
      ];
  ...
  const handleExport = () => {
    if (templates.length === 0) return toast.error("No data to export");
    const headers = ["ID", "Name", "Details", "Category"];
    const csvContent = [
      headers.join(","),
      ...templates.map(t => [t.id, `"${t.name}"`, `"${t.details}"`, `"${t.category}"`].join(","))
    ].join("\n");
  ...
  key: "s_no", 
  label: "S.No", 
  render: (_: EmailTemplate, index: number) => {
    return (
      ...
      render: (item) => (
        <div className="max-w-[500px] py-1">
          <p className="text-[12px] font-medium text-gray-500 dark:text-gray-400 leading-relaxed line-clamp-2 italic">
            {item.details || "No content provided"}
          </p>
        </div>
      )
    },
  ...
  const filteredData = useMemo(() => {
    const dataToFilter = Array.isArray(templates) ? templates : [];
    ...
    if (sortConfig.key && sortConfig.order) {
      result.sort((a, b) => {
        const aVal = String(a[sortConfig.key as keyof EmailTemplate]).toLowerCase();
        const bVal = String(b[sortConfig.key as keyof EmailTemplate]).toLowerCase();
    ...
    return result;
  }, [templates, searchQuery, sortConfig, filterCategory]);
      ...
      <PageHeader 
        ...
        rightContent={
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={() => fileInputRef.current?.click()} className="h-10 text-[12px] font-bold gap-2 border-slate-200 dark:border-gray-800 text-slate-600 hover:bg-slate-50 transition-all rounded-xl shadow-sm uppercase tracking-wider bg-white">
              <Upload size={15} strokeWidth={2.5} /> Import
            </Button>
            <Button variant="outline" onClick={handleExport} className="h-10 text-[12px] font-bold gap-2 border-slate-200 dark:border-gray-800 text-slate-600 hover:bg-slate-50 transition-all rounded-xl shadow-sm uppercase tracking-wider bg-white">
      ...
      <DataTableSearch 
        ...
        filterContent={
          <div className="space-y-4">
             ...
             <Button variant="ghost" onClick={() => setFilterCategory("All")} className="w-full h-10 rounded-xl text-[11px] font-black uppercase tracking-widest text-rose-500 hover:bg-rose-50 transition-all font-bold">
                Reset Filters
             </Button>
          </div>
        }
