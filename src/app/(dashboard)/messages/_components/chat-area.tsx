...
const MOCK_CHATS_HISTORY: Record<number, { date: string, messages: any[] }> = {
  1: {
    date: '14 AUG 2019',
    messages: [
      {
  ...
  3: {
    ...
    messages: [
      ...
      },
      {
        id: 8,
        type: 'sent',
        content: 'Nam quam nunc, blandit vel, luctus pulvinar, hendrerit id, lorem. Maecenas nec odio et ante tincidunt tempus. Donec vitae sapien ut libero venenatis faucibus.',
        time: '09:40 am',
        isFirstInGroup: false
      }
    ]
...
export function ChatArea({ selectedChat }: ChatAreaProps) {
  ...
  if (!selectedChat) {
    return (
      <div className={`flex-1 flex flex-col items-center justify-center ${cardClass} bg-card/50 backdrop-blur-sm p-6 text-center`}>
            ...
            >
              <div className={`flex ${isSent ? 'justify-end' : 'justify-start'} items-center gap-3 w-full`}>
                ...
                <div className="flex flex-col gap-2 max-w-[70%]">
                  <div className={`p-[14px_18px] rounded-[5px] text-[13.5px] leading-relaxed ${
                    isSent 
              ...
              {msg.isFirstInGroup && (
                <span className={`text-muted-foreground text-[11px] ${isSent ? 'pr-[48px] text-right' : 'pl-[48px] text-left'} pt-1 font-medium w-full tracking-wide`}>
                  {msg.time}
                </span>
              )}
            </div>
