    ...
    <div className="h-[calc(100vh-86px)] overflow-y-auto px-6 py-8 custom-scrollbar bg-[#F8FAFC] dark:bg-black/40">
      ...
      <div className="max-w-[1700px] mx-auto mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--vendor-text-muted)] font-medium tracking-tight">Configure your audience and select a template to start your campaign.</p>
        </div>
        <Link href="/newsletter/email">
          <button className="flex items-center gap-2 text-[var(--vendor-text-muted)] hover:text-gray-900 dark:hover:text-white transition-all font-bold text-[11px] uppercase tracking-[0.2em] hover:bg-transparent">
            <ArrowLeft size={16} className="text-[var(--vendor-primary-btn)]" strokeWidth={3} /> Back to Email
          </button>
        </Link>
      </div>
      ...
      <div className="max-w-[1700px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-in fade-in slide-in-from-bottom duration-1000 pb-10">
        ...
        <div className="lg:col-span-9 relative z-0">
          <div className="bg-white dark:bg-sidebar p-8 md:p-10 rounded-[var(--vendor-radius-panel)] shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border-none">
            <div className="grid grid-cols-3 gap-8">
              ...
              <div className="col-span-2 space-y-10">
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-5">
                    <Label className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide">Send To</Label>
                    <div className="inline-flex gap-1 p-1 bg-gray-100 dark:bg-white/5 rounded-[var(--vendor-radius-control)]">
                      {([
                        ...
                        return (
                          <button key={value} type="button" onClick={() => setFrom(value)}
                            className={cn(
                              "flex items-center gap-2 px-4 py-2.5 rounded-[var(--vendor-radius-control)] text-[var(--vendor-control-text)] font-semibold uppercase tracking-wide transition-all duration-200",
                              isActive ? "bg-white dark:bg-sidebar shadow-sm text-gray-800 dark:text-white" : "text-[var(--vendor-text-muted)] hover:text-gray-600 dark:hover:text-gray-300"
                            )}
                          >
                            <Icon size={14} className={isActive ? "text-[var(--vendor-primary-btn)]" : ""} />
                            {label}
                          </button>
                        );
                      })}
                  ...
                  {/* User Type */}
                  <div className="space-y-5">
                    <Label className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide">User Type</Label>
                    <div className="inline-flex gap-1 p-1 bg-gray-100 dark:bg-white/5 rounded-[var(--vendor-radius-control)]">
                      {USER_TYPE_OPTIONS.map((opt) => {
                        ...
                        return (
                          <button key={opt.value} type="button" onClick={() => setUserType(opt.value)}
                            className={cn(
                              "flex items-center gap-2 px-5 py-2.5 rounded-[var(--vendor-radius-control)] text-[var(--vendor-control-text)] font-semibold uppercase tracking-wide transition-all duration-200",
                              isActive ? "bg-white dark:bg-sidebar shadow-sm text-gray-800 dark:text-white" : "text-[var(--vendor-text-muted)] hover:text-gray-600 dark:hover:text-gray-300"
                            )}
                          >
                            <Icon size={14} className={isActive ? "text-[var(--vendor-primary-btn)]" : ""} />
                ...
                <div className="space-y-5">
                  <Label className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide">Plan Filter</Label>
                  <Popover>
                    <PopoverTrigger asChild disabled={userType === "Guest"}>
                      <button className="min-h-12 w-full px-4 py-2 rounded-[var(--vendor-radius-control)] text-sm font-bold border-[var(--vendor-border)] dark:border-white/10 bg-gray-50 dark:bg-white/5 transition-all border text-left flex items-center justify-between gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-white/10">
                        {selectedPlans.length === 0 ? (
                          <span className="text-[var(--vendor-text-muted)]">All Plans</span>
                        ) : (
                        ...
                        <ChevronDown size={16} className="text-[var(--vendor-text-muted)] flex-shrink-0" />
                      </button>
                    </PopoverTrigger>
                    <PopoverContent className="w-64 p-2 rounded-[var(--vendor-radius-control)] border-[var(--vendor-border)] dark:border-white/10">
                      <div className="space-y-1 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                        {planOptions.length === 0 ? (
                          <p className="text-xs text-[var(--vendor-text-muted)] px-3 py-2">No plans available</p>
                        ) : planOptions.map(o => {
                          const isSelected = selectedPlans.includes(o.name);
                          return (
                            <button key={o.id} onClick={() => togglePlan(o.name)}
                              className={cn(
                                "w-full flex items-center gap-2 px-3 py-2.5 rounded-[var(--vendor-radius-control)] text-sm font-bold transition-all text-left",
                                isSelected ? "bg-blue-50 dark:bg-blue-500/20" : "hover:bg-gray-100 dark:hover:bg-white/10"
                              )}
                            >
                              <div className={cn(
                                "w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all",
                                isSelected ? "bg-blue-500 border-[var(--vendor-primary-btn)]" : "border-gray-300 dark:border-gray-600"
                              )}>
                                {isSelected && <Check size={10} className="text-white" />}
                              </div>
                              <span
                                className="text-[var(--vendor-control-text)] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full truncate"
                                style={o.label_color
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="w-full">
                      ...
                      >
                        <SelectTrigger className="h-12 w-full rounded-[var(--vendor-radius-control)] text-sm font-bold border-[var(--vendor-border)] dark:border-white/10 bg-gray-50 dark:bg-white/5 focus:ring-4 focus:ring-violet-500/10 transition-all">
                          <SelectValue placeholder={catsLoading ? "Loading..." : "Sample Category"} />
                        </SelectTrigger>
                        <SelectContent className="rounded-[var(--vendor-radius-control)] font-bold border-[var(--vendor-border)] dark:border-white/10">
                          {categories.length === 0 ? (
                  ...
                  {/* Email Template */}
                  <div className="space-y-5">
                    <Label className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide">Template</Label>
                    <div className="w-full">
                      <Select
                      ...
                      >
                        <SelectTrigger className="h-12 w-full rounded-[var(--vendor-radius-control)] text-sm font-bold border-[var(--vendor-border)] dark:border-white/10 bg-gray-50 dark:bg-white/5 focus:ring-4 focus:ring-emerald-500/10 transition-all">
              ...
              {/* Live Stats - 1 col */}
              <div className="col-span-1 border-l border-[var(--vendor-border)] dark:border-gray-700 pl-8">
                <p className="text-[var(--vendor-form-label-text)] font-semibold uppercase text-[var(--vendor-text-muted)] tracking-wide mb-6">Live Stats</p>
                <div className="space-y-6">
                  {/* Recipients */}
                  <div className="space-y-2">
                    <p className="text-2xl font-bold text-[var(--vendor-text)]">{stats.totalRecipients.toLocaleString()}</p>
                    <p className="text-xs text-gray-600 dark:text-[var(--vendor-text-muted)] font-medium">Recipients</p>
                  </div>
                  <div className="h-px bg-gray-200 dark:bg-gray-700"></div>

                  {/* Plan-wise counts */}
                  <div className="space-y-2">
                    <p className="text-xs text-gray-600 dark:text-[var(--vendor-text-muted)] font-medium mb-3">By Plan</p>
                    {userType === "Guest" ? (
                      <p className="text-sm font-bold text-[var(--vendor-text-muted)]">—</p>
                    ) : planOptions.length === 0 ? (
                      <p className="text-sm font-bold text-[var(--vendor-text-muted)]">No plans</p>
                    ) : (
                      <div className="space-y-2">
                        {planOptions.map(p => {
                          ...
                          return (
                            <div key={p.id} className="flex items-center justify-between gap-2">
                              <span
                                className="text-[10px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full truncate max-w-[100px]"
                                style={p.label_color
                  ...
                  {/* Segment */}
                  <div className="space-y-2">
                    <p className="text-lg font-bold text-[var(--vendor-text)]">{stats.segment}</p>
                    <p className="text-xs text-gray-600 dark:text-[var(--vendor-text-muted)] font-medium">Segment</p>
                  </div>
                </div>
              </div>
